...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>Halaman Order Posting Driver</h1>
        </div>
        @if (session('success'))
            ...
            <div class="row p-3">
                <div class="card col-12">
                    <div class="card-header">
                        <div class="section-title mt-0 mb-0">Data Order Posting Driver</div>
                            ...
                            <tbody>
                                ...
                                @foreach ($data as $data)
                                    <tr>
                                        ...
                                        <td>{{ $data->posting->judul_posting }}</td>
                                        <td>{{ $data->jumlah_pesanan }}</td>
                                        <td>{{ $data->ongkir }}</td>
                                        <td>{{ $data->total_harga }}</td>
                                        @if($data->status_order == 3)
                                            <td><span class="badge badge-info">Menyiapkan Pesanan</span></td>
            ...
            <form action="" id="deleteForm" method="post">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        {{ csrf_field() }}
                        {{ method_field('POST') }}
                        <p>Apakah anda yakin ingin menghapus Orderan Posting ini ?</p>
...
@section('scripts')
    ...
    <script type="text/javascript">
        function deleteData(id) {
            var id = id;
            var url = '{{route("order-posting.delete", ":id") }}';
